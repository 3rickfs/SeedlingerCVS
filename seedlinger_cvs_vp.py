...
class calidad:
    ...
    def run(self,agujero=0):
        ...
        h_predictions, _ = self.__call_yolo_predict("h", h_img)
        ...
        v_predictions, v_pmask = self.__call_yolo_predict("v", v_img, v_mask)
        ...
        # Print prediction info
        #self.print_prediction_info(v_predictions, v_img, 'vertical')
        self.print_prediction_info_VERTICAL(v_predictions,v_img, v_mask, 'vertical')
        

        #Get type of seedling
        tos = self.__get_type_of_seedling(h_predictions, v_predictions, v_pmask)
        # Save h and v images
        self.__save_image_proc(h_img, v_img, agujero, tos)
        self.__save_image(s_h_img, s_v_img, v_mask, agujero, tos) 

        return tos


    def __call_yolo_predict(self,axis, img, mask=None):
    ...
    def __get_type_of_seedling(self,hp, vp, vm):
        global linear
    ...
    def __save_image(self,h_img, v_img, mask=None,agujero=0,calidad=0):
        try:
    ...
    def __save_image_proc(self,h_img, v_img,agujero=0,calidad=0):
        try:
            cdt = str(datetime.now())
            fn = cdt.replace(" ", "_")
            fn = fn.replace(":", "-")
